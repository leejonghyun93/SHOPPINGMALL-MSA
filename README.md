...
### 팀 구성 (3명)
- **팀장**: [이종현] - MSA 아키텍처 설계 및 백엔드 전체 개발
- **팀원 1**: [이동주] - 사용자 화면 라이브 방송 구현
- **팀원 2**: [검석률] - 사용자 화면 실시간 채팅 구현
...
- **Live Streaming Service**: 실시간 방송 관리, 방송 알림 스케줄링
...
- **상품 연동**: 방송 중 상품 구매 연결
- **방송 알림**: 방송 시작 시 구독자에게 이메일 알림 자동 발송
...
### 스케줄링 시스템
- **Spring Scheduling**: @Scheduled 어노테이션 기반 주기적 작업 처리
- **방송 알림 배치**: 30초마다 시작 예정 방송에 대한 알림 자동 발송
- **방송 리마인더**: 1분마다 5분 후 시작 방송 확인 및 리마인더 처리
- **데이터 정리**: 매일 새벽 2시 오래된 알림 데이터 자동 아카이빙
- **통계 수집**: 1시간마다 알림 발송 통계 로그 기록
- **트랜잭션 관리**: @Transactional 기반 안전한 배치 처리
...
- **이벤트 드리븐**: 방송 알림, 회원 탈퇴, 주문 처리 등 이벤트 기반 처리
- **장애 격리**: 서비스 간 느슨한 결합으로 장애 전파 방지
- **Kafka Topics**:
    - `notification-events`: 알림 이벤트 메시지 큐
    - `live-broadcast-notification-topic`: 라이브 방송 알림 전용 토픽
...
#### 팀장 - 이종현
**MSA 아키텍처 설계 및 백엔드 전체 개발**
- **MSA 전체 설계**: 서비스 분리, 아키텍처 구성, 통신 방식 설계
- **Config Service**: Spring Cloud Config 중앙 설정 서버 구축
- **Eureka Service**: 서비스 디스커버리 및 등록 시스템 구현
- **Auth Service**: JWT 인증, 소셜 로그인 (카카오/네이버) 시스템 개발
- **User Service**: 사용자 관리, 세션 처리, Redis 캐싱 구현
- **Commerce Service**: 상품 관리, 주문/결제 시스템, Import PG 연동
- **Live Streaming Service**: 실시간 방송 관리 백엔드 서비스
- **API Gateway**: Spring Cloud Gateway 라우팅 및 필터 구현
- **Spring Scheduling**: @Scheduled 기반 방송 알림 자동화 시스템 개발
- **Kafka 메시징**: 이벤트 기반 알림 시스템 구현 (Producer/Consumer)
- **이메일 알림**: Gmail SMTP 연동 및 HTML 템플릿 이메일 발송
- **CI/CD 파이프라인**: GitHub Actions, AWS ECR/EC2 배포 자동화
- **데이터베이스 설계**: MariaDB ERD 설계 및 JPA/MyBatis 구현
- **Redis 캐싱**: 세션 관리 및 성능 최적화
- **Docker 인프라**: 전체 서비스 컨테이너화 및 오케스트레이션

#### 팀원 1 - 이동주
**사용자 화면 - 라이브 방송 화면**
- **라이브 방송 시청**: Vue.js 기반 실시간 스트리밍 시청 UI 구현
- **방송 목록**: 카테고리별 방송 목록 조회 화면
- **HLS 플레이어**: 브라우저 기반 영상 스트리밍 재생 기능
- **방송 상세**: 방송 정보 및 상품 연동 화면

#### 팀원 2 - 검석률
**사용자 화면 - 실시간 채팅**
- **실시간 채팅**: WebSocket + STOMP 프로토콜 기반 채팅 인터페이스 구현
- **채팅 UI**: 방송 시청 중 실시간 메시지 송수신 화면
- **채팅 기능**: 메시지 전송, 사용자 목록, 채팅 히스토리

### 협업 방식
- **Git 브랜치 전략**: feature → develop → main 브랜치 관리
- **코드 리뷰**: Pull Request 기반 상호 코드 검토
- **일일 스탠드업**: 진행 상황 공유 및 이슈 해결
- **API 문서화**: Swagger를 통한 API 명세 공유
...
### 방송 알림 발송 지연 문제
- **문제**: 방송 시작 시 알림이 5분 이상 지연되어 발송됨
- **해결**: @Scheduled 주기를 30초로 단축, Kafka를 통한 비동기 처리 도입
- **결과**: 방송 시작 후 1분 이내 알림 발송 성공

### 이메일 발송 실패 문제
- **문제**: Gmail SMTP 인증 실패로 이메일 발송 불가
- **해결**:
    - Gmail 앱 비밀번호 재생성
    - 환경변수 기반 설정으로 변경 (PROD_MAIL_USERNAME, PROD_MAIL_PASSWORD)
    - @PostConstruct로 설정 검증 로직 추가
- **결과**: 안정적인 이메일 알림 발송 시스템 구축
...
- Kubernetes 마이그레이션으로 오케스트레이션 고도화
- ELK 스택 도입으로 로그 분석 시스템 구축
- 알림 시스템 확장 (SMS, 푸시 알림)
- 스케줄링 작업 모니터링 대시보드 구축