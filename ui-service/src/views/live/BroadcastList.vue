...
const getBroadcastThumbnail = (broadcast) => {
  ...
  if (thumbnailUrl && thumbnailUrl.trim() !== '') {
    ...
    if (thumbnailUrl.startsWith('/upload/') && !thumbnailUrl.startsWith('/upload/product/main/')) {
      return thumbnailUrl // 직접 반환
    }

    // 기존 DB 경로인 경우 (그대로 유지하여 두 경로 모두 지원)
    if (thumbnailUrl.startsWith('/upload/product/main/')) {
      return thumbnailUrl // 그대로 반환
    }
