...
public class BroadcastServiceClient {
    ...
    public BroadcastInfo getBroadcastInfo(Long broadcastId) {
        try {
            ...
            BroadcastInfo broadcastInfo = BroadcastInfo.builder()
                    ...
                    .broadcasterId(broadcasterId)  // String 타입
                    .build();
    ...
    /**
     * broadcaster_id를 USER_ID로 변환 (String 타입 처리)
    ...
    private String getBroadcasterUserId(String broadcasterId) {
        ...
        try {
            // 🔥 실제 운영: broadcaster_id를 그대로 USER_ID로 사용
            ...
            // 🔥 숫자로 된 broadcaster_id인 경우 그대로 사용
            log.warn("broadcaster_id {}에 해당하는 USER_ID를 찾을 수 없습니다", broadcasterId);
            return broadcasterId;  // 그대로 반환

        } catch (Exception e) {
            log.error("broadcaster_id {} 변환 실패: {}", broadcasterId, e.getMessage());
            return broadcasterId;  // 오류시 그대로 반환
        }
    }

    /**
     * 방송자 이름 조회 (String broadcasterId 처리)
     */
    ...
    public String getBroadcasterName(Long broadcastId) {
        try {
            String broadcasterId = broadcastRepository.findBroadcasterIdByBroadcastId(broadcastId)
                    .orElse(null);
    ...
    /**
     * 특정 broadcaster_id의 방송 목록 조회 (String 타입 처리)
     */
    ...
    /**
     * 방송 정보 DTO (broadcasterId를 String으로 변경)
     */
    public static class BroadcastInfo {
        ...
        public String broadcasterId;     // 방송자 ID (String으로 변경)
        ...
        public static class BroadcastInfoBuilder {
            ...
            public BroadcastInfoBuilder broadcasterId(String broadcasterId) {  // String으로 변경
